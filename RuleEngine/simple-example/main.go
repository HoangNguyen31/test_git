...
func printStatistics(userList []*User) {
	...
	for _, user := range userList {
		...
		if user.Membership == "DIAMOND" && user.Discount != "" && user.Gender != "WOMAN" {
			diamondDiscountCount++
		}
	...
	fmt.Printf("Number of diamond members with discount: %d\n", diamondDiscountCount)
...
func main() {
	nameList := []string{"Alice", "Bob", "Charlie", "David", "Emma", "Frank", "James", "Peter", "Chris"}
	genderList := []string{"MAN", "WOMAN"}
	membershipList := []string{"DIAMOND", "GOLD", "SILVER", "BRONZE", "NEW"}
